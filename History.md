...
* Features
  * Add an API to update min and max thread counts at runtime ([#3658])
  * Add single-mode and cluster-mode configuration hooks ([#3621])
  * Add `env["puma.mark_as_io_bound"]` to mark requests as IO-bound ([#3816], [#3894])
  * Add an `on_force` option to `shutdown_debug` for forced-shutdown handling ([#3671])

* Bugfixes
  * Enable backtrace dumping via `SIGPWR` on Linux with JRuby ([#3829])
  * Improve HTTP parser behavior on JRuby ([#3838])
  * Fix phased restart with `fork_worker` so new workers are not forked from a stale worker ([#3853])

* Performance
  * Reduce header processing overhead by caching downcased header keys ([#3874])

* Docs
  * Add a guide for running gRPC with Puma ([#3885])

* CI
  * Expand and stabilize the test matrix across Rubies, including parallel integration runs and JRuby-specific test fixes ([#3866], [#3870], [#3873], [#3876], [#3878], [#3879], [#3880])
